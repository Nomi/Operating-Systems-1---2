...
int main(int argc, char** argv)
{
    ...
    pthread_mutex_t** mtxArrayBeingRead = malloc(sizeof(pthread_mutex_t*)*N);
...
int KingKongWork(const int N, const int T, sigThreadArg* pSigThr, pthread_mutex_t** pMtxArrayBeingRead, int* arrNYC)
{
    ...
    do
    {
        ...
        pthread_mutex_lock(pSigThr->mtxSigIntRecv);
        for(int timeSec=0;kkIndex<N-1&& timeSec<T && *pSigThr->sigIntRecvd==0; timeSec+=1) //N-1 is the last possible index, so we break out once we reach there.
        {
            pthread_mutex_unlock(pSigThr->mtxSigIntRecv);
